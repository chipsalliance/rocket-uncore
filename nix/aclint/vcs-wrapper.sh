...
_ACLINT_SIM_RESULT_DIR=${ACLINT_SIM_RESULT_DIR:-"aclint-sim-result"}
_CURRENT="$_ACLINT_SIM_RESULT_DIR"/all/"$_NOW"
mkdir -p "$_CURRENT"
ln -sfn "all/$_NOW" "$_ACLINT_SIM_RESULT_DIR/result"
...
echo "VCS emulator finished, result saved in $_ACLINT_SIM_RESULT_DIR/result"
