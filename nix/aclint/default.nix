...
let
  designTarget = "ACLINT";
  tbTarget = "ACLINTTestBench";
  dpiLibName = "aclintemu";
in
{
  # RTL
  aclint-compiled = scope.callPackage ./aclint.nix { target = designTarget; };
  elaborate = scope.callPackage ./elaborate.nix {
    elaborator = scope.aclint-compiled.elaborator;
  };
  mlirbc = scope.callPackage ./mlirbc.nix { };
  rtl = scope.callPackage ./rtl.nix { };

  # Testbench
  tb-compiled = scope.callPackage ./aclint.nix { target = tbTarget; };
