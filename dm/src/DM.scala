...
class TLDebugModuleInnerAsync(val parameter: DMParameter)
  ...
  when(dmAuthenticated) {
    ...
    when(~DMSTATUSRdData.allnonexistent) { // if no existent harts selected, all other status is false
      ...
      DMSTATUSRdData.anyhalted := ((~io.debugUnavail.asUInt & (haltedBitRegs)) & hamaskFull.asUInt).asBools
        .reduce(_ | _)
      DMSTATUSRdData.anyrunning := ((~io.debugUnavail.asUInt & ~(haltedBitRegs)) & hamaskFull.asUInt).asBools
        .reduce(_ | _)
      ...
      when(~DMSTATUSRdData.anynonexistent) { // if one hart is nonexistent, no 'all' status is set
        ...
        DMSTATUSRdData.allhalted := ((~io.debugUnavail.asUInt & (haltedBitRegs)) | ~hamaskFull.asUInt).asBools
          .reduce(_ & _)
        DMSTATUSRdData.allrunning := ((~io.debugUnavail.asUInt & ~(haltedBitRegs)) | ~hamaskFull.asUInt).asBools
          .reduce(_ & _)
