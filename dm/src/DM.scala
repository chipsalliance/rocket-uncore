...
class DMInterface(parameter: DMParameter) extends Bundle {
  ...
  val interrupt = Output(RegInit(VecInit(Seq.fill(parameter.nComponents) { false.B })))
...
class DM(val parameter: DMParameter)
  ...
  val supportHartArray = parameter.supportHartArray && (parameter.nComponents > 1) // no hart array if only one hart
  val nExtTriggers = parameter.nExtTriggers
  val nHaltGroups =
    if ((parameter.nComponents > 1) | (parameter.nExtTriggers > 0)) parameter.nHaltGroups
    else 0 // no halt groups possible if single hart with no external triggers

  val hartSelFuncs =
    if (parameter.nComponents > 1)
  ...
  require(parameter.supportQuickAccess == false, "No Quick Access support yet")
  ...
  val haltedBitRegs = Reg(UInt(parameter.nComponents.W))

  /** all harts resume request status */
  val resumeReqRegs = Reg(UInt(parameter.nComponents.W))

  /** all harts have reset status */
  val haveResetBitRegs = Reg(UInt(parameter.nComponents.W))

  // default is 1,after resume, resumeAcks get 0
  /** all harts resume ack status */
  val resumeAcks = Wire(UInt(parameter.nComponents.W))
  ...
  val dmiProgramBufferRdEn = WireInit(VecInit(Seq.fill(parameter.nProgramBufferWords * 4) { false.B }))
  val dmiProgramBufferAccessLegal = WireInit(false.B)
  val dmiProgramBufferWrEnMaybe = WireInit(VecInit(Seq.fill(parameter.nProgramBufferWords * 4) { false.B }))

  val dmiAbstractDataRdEn = WireInit(VecInit(Seq.fill(parameter.nAbstractDataWords * 4) { false.B }))
  val dmiAbstractDataAccessLegal = WireInit(false.B)
  val dmiAbstractDataWrEnMaybe = WireInit(VecInit(Seq.fill(parameter.nAbstractDataWords * 4) { false.B }))

  // --------------------------------------------------------------
  // Registers coming from 'CONTROL' in Outer
  // --------------------------------------------------------------

  // val i_dmAuthenticated = io.auth.map(a => a.dmAuthenticated).getOrElse(true.B)
  val i_dmAuthenticated = true.B

  val selectedHartReg = Reg(UInt(parameter.MaxHartIdBits.W))
  // hamaskFull is a vector of all selected harts including hartsel, whether or not supportHartArray is true
  val hamaskFull = WireInit(VecInit(Seq.fill(parameter.nComponents) { false.B }))

  if (parameter.nComponents > 1) {
    when(~io.ctrl.dmactive) {
      selectedHartReg := 0.U
    }.elsewhen(innerCtrl.fire) {
      selectedHartReg := innerCtrl.bits.hartsel
    }
  }

  if (supportHartArray) {
    val hamaskZero = WireInit(VecInit(Seq.fill(parameter.nComponents) { false.B }))
    val hamaskReg = Reg(Vec(parameter.nComponents, Bool()))
    when(~io.ctrl.dmactive || ~i_dmAuthenticated) {
      hamaskReg := hamaskZero
    }.elsewhen(innerCtrl.fire) {
      hamaskReg := Mux(innerCtrl.bits.hasel, innerCtrl.bits.hamask, hamaskZero)
    }
    hamaskFull := hamaskReg
  }
  // Outer.hamask doesn't consider the hart selected by dmcontrol.hartsello,
  // so append it here
  when(selectedHartReg < parameter.nComponents.U) {
    hamaskFull(if (parameter.nComponents == 1) 0.U(0.W) else selectedHartReg) := true.B
  }

  innerCtrl.ready := true.B

  // Construct a Vec from innerCtrl fields indicating whether each hart is being selected in this write
  // A hart may be selected by hartsel field or by hart array
  val hamaskWrSel = WireInit(VecInit(Seq.fill(parameter.nComponents) { false.B }))
  for (component <- 0 until parameter.nComponents) {
    hamaskWrSel(component) := ((innerCtrl.bits.hartsel === component.U) ||
      (if (supportHartArray) innerCtrl.bits.hasel && innerCtrl.bits.hamask(component) else false.B))
  }
  ...
  // -------------------------------------
  val hrReset = WireInit(VecInit(Seq.fill(parameter.nComponents) { false.B }))
  val hrDebugInt = Wire(Vec(parameter.nComponents, Bool()))
  val i_hrmaskReg = RegInit(hrReset)
  val hartIsInResetSync = Wire(Vec(parameter.nComponents, Bool()))
  ...
  when(~io.ctrl.dmactive || ~i_dmAuthenticated) {
    ...
    i_hrmaskReg := innerCtrl.bits.hrmask
  }

  // withReset(reset.asAsyncReset) {          // ensure interrupt requests are negated at first clock edge
  val hrDebugIntReg = RegInit(VecInit(Seq.fill(parameter.nComponents) { false.B }))
  when(~io.ctrl.dmactive || ~i_dmAuthenticated) {
  ...
  DMSTATUSRdData.authbusy := false.B

  val resumereq = innerCtrl.fire && innerCtrl.bits.resumereq

  when(i_dmAuthenticated) {
    DMSTATUSRdData.hasresethaltreq := true.B

    DMSTATUSRdData.anynonexistent := (selectedHartReg >= parameter.nComponents.U) // only hartsel can be nonexistent

    // all harts nonexistent if hartsel is out of range and there are no harts selected in the hart array
    DMSTATUSRdData.allnonexistent := (selectedHartReg >= parameter.nComponents.U) & (~hamaskFull.reduce(_ | _))

    when(~DMSTATUSRdData.allnonexistent) { // if no existent harts selected, all other status is false
      DMSTATUSRdData.anyunavail := (io.ctrl.debugUnavail.asUInt & hamaskFull.asUInt).asBools.reduce(_ | _)
      DMSTATUSRdData.anyhalted := ((~io.ctrl.debugUnavail.asUInt & (haltedBitRegs)) & hamaskFull.asUInt).asBools
        .reduce(_ | _)
      DMSTATUSRdData.anyrunning := ((~io.ctrl.debugUnavail.asUInt & ~(haltedBitRegs)) & hamaskFull.asUInt).asBools
        .reduce(_ | _)
      ...
      when(~DMSTATUSRdData.anynonexistent) { // if one hart is nonexistent, no 'all' status is set
        DMSTATUSRdData.allunavail := (io.ctrl.debugUnavail.asUInt | ~hamaskFull.asUInt).asBools.reduce(_ & _)
        DMSTATUSRdData.allhalted := ((~io.ctrl.debugUnavail.asUInt & (haltedBitRegs)) | ~hamaskFull.asUInt).asBools
          .reduce(_ & _)
        DMSTATUSRdData.allrunning := ((~io.ctrl.debugUnavail.asUInt & ~(haltedBitRegs)) | ~hamaskFull.asUInt).asBools
          .reduce(_ & _)
    ...
    DMSTATUSRdData.impebreak := (parameter.hasImplicitEbreak).B
  }

  when(~io.ctrl.dmactive || ~i_dmAuthenticated) {
    haveResetBitRegs := 0.U
  }.otherwise {
    when(innerCtrl.fire && innerCtrl.bits.ackhavereset) {
  ...
  val i_hgDebugInt = WireInit(VecInit(Seq.fill(parameter.nComponents) { false.B }))
  ...
  if (nHaltGroups > 0) { // TODO: async reset ensures triggers don't falsely fire during startup
    ...
    val hgParticipateHart = RegInit(VecInit(Seq.fill(parameter.nComponents)(0.U(hgBits.W))))
    ...
    DMCS2RdData.haltgroup := hgParticipateHart(if (parameter.nComponents == 1) 0.U(0.W) else selectedHartReg)
    ...
    // io.extTrigger.foreach { extTrigger =>
    //   val extTriggerOutReq = RegInit(VecInit(Seq.fill(parameter.nExtTriggers) { false.B }))
  ...
  hgDebugInt := i_hgDebugInt.asUInt | hrDebugInt.asUInt

  // ----HALTSUM*
  val numHaltedStatus = ((parameter.nComponents - 1) / 32) + 1
  ...
  ABSTRACTCSReset.datacount := parameter.nAbstractDataWords.U
  ABSTRACTCSReset.progbufsize := parameter.nProgramBufferWords.U
  ...
  when(~io.ctrl.dmactive || ~i_dmAuthenticated) {
    ...
    when(autoexecprogbufWrEnMaybe && ABSTRACTAUTOWrEnLegal) {
      ABSTRACTAUTOReg.autoexecprogbuf := ABSTRACTAUTOWrData.autoexecprogbuf & ((1 << parameter.nProgramBufferWords) - 1).U
    }
    when(autoexecdataWrEnMaybe && ABSTRACTAUTOWrEnLegal) {
      ABSTRACTAUTOReg.autoexecdata := ABSTRACTAUTOWrData.autoexecdata & ((1 << parameter.nAbstractDataWords) - 1).U
    }
  }

  // Abstract Data access vector(byte-addressable)
  val dmiAbstractDataAccessVec = WireInit(VecInit(Seq.fill(parameter.nAbstractDataWords * 4) { false.B }))
  ...
  val dmiProgramBufferAccessVec = WireInit(VecInit(Seq.fill(parameter.nProgramBufferWords * 4) { false.B }))
  ...
  val autoexecData = WireInit(VecInit(Seq.fill(parameter.nAbstractDataWords) { false.B }))
  val autoexecProg = WireInit(VecInit(Seq.fill(parameter.nProgramBufferWords) { false.B }))
  ...
  val abstractDataMem = Reg(Vec(parameter.nAbstractDataWords * 4, UInt(8.W)))
  val abstractDataNxt = WireInit(abstractDataMem)

  // --- Program Buffer

  // byte-addressible mem
  val programBufferMem = Reg(Vec(parameter.nProgramBufferWords * 4, UInt(8.W)))
  ...
  when(~io.ctrl.dmactive || ~i_dmAuthenticated) {
    ...
    val hartselIndex = UIntToOH(innerCtrl.bits.hartsel)
    when(hartHaltedWrEn) {
  ...
  val dmstatusRegFields = RegFieldGroup(
    ...
    Seq(
      ...
      RegField.r(
        ...
        RegFieldDesc("impebreak", "impebreak", reset = Some(if (parameter.hasImplicitEbreak) 1 else 0))
      )
    )
  )
  ...
  val abstractcsRegFields = RegFieldGroup(
    ...
    Seq(
      RegField.r(
        ...
        RegFieldDesc("datacount", "number of DATA registers", reset = Some(parameter.nAbstractDataWords))
      ),
      RegField(4),
      WNotifyVal(
        3,
  ...
  // )
  regmap(
    dmiInnerNode.viewAs[AXI4RWIrrevocable],
    0,
    false,
    ...
    (DMI_ABSTRACTAUTO << 2) -> RegFieldGroup(
      ...
      Seq(
        ...
        RegField(16 - parameter.nAbstractDataWords),
        WNotifyVal(
          parameter.nProgramBufferWords,
    ...
    (DMI_AUTHDATA << 2) -> (if (parameter.hasAuthentication)
                              RegFieldGroup(
                                "dmi_authdata",
  ...
  jalAbstract.setImm(ABSTRACT(parameter) - WHERETO)
  ...
  require(
    (parameter.atzero && parameter.nAbstractInstructions == 2) || (!parameter.atzero && parameter.nAbstractInstructions == 5),
    "Mismatch between DMParameter atzero and nAbstractInstructions"
  )
  val abstractGeneratedMem = Reg(Vec(parameter.nAbstractInstructions, (UInt(32.W))))
  ...
  when(goAbstract) {
    if (parameter.nAbstractInstructions == 2) {
      ...
      abstractGeneratedMem(0) := Mux(
        ...
        Mux(accessRegisterCommandReg.write, abstractGeneratedI(parameter), abstractGeneratedS(parameter)),
        nop.asUInt
      )
  ...
  regmap(
    io.node.viewAs[AXI4RWIrrevocable],
    0,
    false,
    ...
    PROGBUF(parameter) -> RegFieldGroup(
      "debug_progbuf",
    ...
    IMPEBREAK(parameter) -> {
      if (parameter.hasImplicitEbreak)
        Seq(
          RegField.r(
            32,
    ...
    ABSTRACT(parameter) -> RegFieldGroup(
      "debug_abstract",
    ...
    ROMBASE -> RegFieldGroup(
      "debug_rom",
      Some("Debug ROM"),
      (if (parameter.atzero) DebugRomContents() else DebugRomNonzeroContents()).zipWithIndex.map { case (x, i) =>
  ...
  val hartHalted = haltedBitRegs(if (parameter.nComponents == 1) 0.U(0.W) else selectedHartReg)
  ...
  assert(
    (!io.ctrl.dmactive || !hartExceptionWrEn || ctrlStateReg === CtrlState(Exec)),
  ...
  withClockAndReset(io.dmi.dmiClock, io.dmi.dmiReset) {
    dmiIO <> io.dmi
  }

  // val o_dmAuthenticated = io.auth.get.dmAuthenticated // TODO: ResetSynchronizerShiftReg
  val o_dmAuthenticated = true.B
  ...
  if (parameter.atzero) when(o_dmAuthenticated) {
    HARTINFORdData.dataaccess := true.B
    HARTINFORdData.datasize := parameter.nAbstractDataWords.U
    HARTINFORdData.dataaddr := DsbRegAddrs.DATA.U
    HARTINFORdData.nscratch := parameter.nScratch.U
  }
  ...
  def hartSelected(hart: Int): Bool = {
    ((innerCtrl.bits.hartsel === hart.U) ||
    (if (supportHartArray) innerCtrl.bits.hasel && innerCtrl.bits.hamask(hart) else false.B))
  }
  ...
  val hartinfoRegFields = RegFieldGroup(
    ...
    Seq(
      RegField.r(
        ...
        RegFieldDesc("dataaddr", "data address", reset = Some(if (parameter.atzero) DsbRegAddrs.DATA else 0))
      ),
      RegField.r(
        4,
        ...
        RegFieldDesc(
          ...
          reset = Some(if (parameter.atzero) parameter.nAbstractDataWords else 0)
        )
      ),
      RegField.r(
        1,
        HARTINFORdData.dataaccess,
        RegFieldDesc("dataaccess", "data access type", reset = Some(if (parameter.atzero) 1 else 0))
      ),
      RegField(3),
      RegField.r(
        4,
        ...
        RegFieldDesc(
          ...
          reset = Some(if (parameter.atzero) parameter.nScratch else 0)
        )
      )
    )
  )
  ...
  for (component <- 0 until nComponents) {
    io.interrupt(component)(0) := debugIntRegs(component) | hgDebugInt(component)
  } // TODO
  ...
  innerCtrlValidReg := innerCtrl.valid & ~innerCtrl.ready // Hold innerctrl request until the async queue accepts it
  innerCtrlResumeReqReg := innerCtrl.bits.resumereq & ~innerCtrl.ready // Hold resumereq until accepted
  innerCtrlAckHaveResetReg := innerCtrl.bits.ackhavereset & ~innerCtrl.ready // Hold ackhavereset until accepted

  innerCtrl.valid := innerCtrlValid | innerCtrlValidReg
  innerCtrl.bits.hartsel := Mux(hartselloWrEn, DMCONTROLWrData.hartsello, DMCONTROLReg.hartsello)
  innerCtrl.bits.resumereq := (resumereqWrEn & DMCONTROLWrData.resumereq) | innerCtrlResumeReqReg
  innerCtrl.bits.ackhavereset := (ackhaveresetWrEn & DMCONTROLWrData.ackhavereset) | innerCtrlAckHaveResetReg
  innerCtrl.bits.hrmask := hrmask
  if (supportHartArray) {
    innerCtrl.bits.hasel := Mux(haselWrEn, DMCONTROLWrData.hasel, DMCONTROLReg.hasel)
    innerCtrl.bits.hamask := hamask
  } else {
    innerCtrl.bits.hasel := DontCare
    innerCtrl.bits.hamask := DontCare
  }
