...
import chisel3._
import chisel3.util.{DecoupledIO, log2Up, Cat, UIntToOH, BitPat}
...
/** Parameters exposed to the top-level design, set based on
  ...
  *  @param nProgramBufferWords Number of 32-bit words for Program Buffer
...
class TLDebugModuleOuterInterface(parameter: DMParameter) extends Bundle {
  ...
  val dmiNode = Input(Flipped(axi4.bundle.verilog.irrevocable(parameter.axi4parameter)).asInstanceOf[AXI4RWIrrevocableVerilog])
}
...
class TLDebugModuleOuter(val parameter: DMParameter)
  ...
  val dmAuthenticated = io.dmAuthenticated.map(
    dma => dma // TODO: ResetSynchronizerShiftReg
  ).getOrElse(true.B)
  ...
  val dmControlRegFields = RegFieldGroup("dmcontrol", Some("debug module control register"), Seq(
    ...
    RegField(12),
    if (nComponents > 1) WNotifyVal(parameter.MaxHartIdBits,
  ...
  regmap(
    io.dmiNode.viewAs[AXI4RWIrrevocable],
    0,
    false,
  ...
  for (component <- 0 until nComponents) {
    io.intnode(component)(0) := debugIntRegs(component) | io.hgDebugInt(component)
  }
...
class TLDebugModuleInnerInterface(parameter: DMParameter) extends Bundle {
  ...
  val dmiNode = Input(Flipped(axi4.bundle.verilog.irrevocable(parameter.axi4parameter)).asInstanceOf[AXI4RWIrrevocableVerilog])
  val tlNode = Input(Flipped(axi4.bundle.verilog.irrevocable(parameter.axi4parameter)).asInstanceOf[AXI4RWIrrevocableVerilog])
...
class TLDebugModuleInner(val parameter: DMParameter)
  ...
  // val customNode = new DebugCustomSink()
    ...
    val nHaltGroups = if ((cfg.nComponents > 1) | (cfg.nExtTriggers > 0)) cfg.nHaltGroups
      else 0  // no halt groups possible if single hart with no external triggers

    val hartSelFuncs = if (cfg.nComponents > 1)  DebugModuleHartSelFuncs (
      hartIdToHartSel = (x) => x,
      hartSelToHartId = (x) => x
    ) else DebugModuleHartSelFuncs(
    ...
    // withReset(reset.asAsyncReset) {          // ensure interrupt requests are negated at first clock edge
      ...
      when (~io.dmactive || ~dmAuthenticated) {
        ...
        hrDebugIntReg := hrmaskReg.asUInt &
    ...
    when (dmAuthenticated) {
      ...
      when (~DMSTATUSRdData.allnonexistent) {  // if no existent harts selected, all other status is false
        DMSTATUSRdData.anyunavail   := (io.debugUnavail.asUInt &  hamaskFull.asUInt).asBools.reduce(_ | _)
        DMSTATUSRdData.anyhalted    := ((~io.debugUnavail.asUInt &  (haltedBitRegs)) &  hamaskFull.asUInt).asBools.reduce(_ | _)
        DMSTATUSRdData.anyrunning   := ((~io.debugUnavail.asUInt & ~(haltedBitRegs)) &  hamaskFull.asUInt).asBools.reduce(_ | _)
        ...
        when (~DMSTATUSRdData.anynonexistent) {  // if one hart is nonexistent, no 'all' status is set
          DMSTATUSRdData.allunavail   := (io.debugUnavail.asUInt | ~hamaskFull.asUInt).asBools.reduce(_ & _)
          DMSTATUSRdData.allhalted    := ((~io.debugUnavail.asUInt &  (haltedBitRegs)) | ~hamaskFull.asUInt).asBools.reduce(_ & _)
          DMSTATUSRdData.allrunning   := ((~io.debugUnavail.asUInt & ~(haltedBitRegs)) | ~hamaskFull.asUInt).asBools.reduce(_ & _)
    ...
    val hgDebugInt     = WireInit(VecInit(Seq.fill(cfg.nComponents) {false.B} ))
    ...
    if (nHaltGroups > 0)  { // TODO: async reset ensures triggers don't falsely fire during startup
      ...
      io.extTrigger.foreach {extTrigger =>
        ...
        for (hg <- 1 to nHaltGroups) {
          hgTrigFiring(hg) := (trigInReq.asUInt & ~(RegNext(trigInReq).asUInt) & VecInit(hgParticipateTrig.map(_ === hg.U)).asUInt).asBools.reduce(_ | _)
          hgTrigsAllAcked(hg) := (trigOutAck.asUInt | VecInit(hgParticipateTrig.map(_ =/= hg.U)).asUInt).asBools.reduce(_ & _)
        }
      ...
      for (hg <- 1 to nHaltGroups) {
        ...
        hgHartsAllHalted(hg) := (haltedBitRegs.asUInt | VecInit(hgParticipateHart.map(_ =/= hg.U)).asUInt).asBools.reduce(_ & _)
    ...
    io.hgDebugInt := hgDebugInt.asUInt | hrDebugInt.asUInt
    ...
    regmap(
      io.tlNode.viewAs[AXI4RWIrrevocable],
      0,
      false,
...
class DMInterface(parameter: DMParameter) extends Bundle {
  ...
  /** Debug Authentication signals from core */
  val auth = Option.when(parameter.hasAuthentication)(new DebugAuthenticationIO())
  val node = Input(Flipped(axi4.bundle.verilog.irrevocable(parameter.axi4parameter)).asInstanceOf[AXI4RWIrrevocableVerilog])
  val intnode = Output(WireInit(VecInit(Seq.fill(parameter.nComponents) {false.B} )))
}
