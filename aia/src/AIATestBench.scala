...
object AIATestBenchParameter {
  implicit def rwP: upickle.default.ReadWriter[AIATestBenchParameter] =
    upickle.default.macroRW
}

/** Parameter of [[AIA]]. */
case class AIATestBenchParameter(
  testVerbatimParameter: TestVerbatimParameter,
  aiaParameter:          AIAParameter,
  timeout:               Int,
  testSize:              Int)
    extends SerializableModuleParameter {
  require(
    (testVerbatimParameter.useAsyncReset && aiaParameter.useAsyncReset) ||
      (!testVerbatimParameter.useAsyncReset && !aiaParameter.useAsyncReset),
...
@instantiable
class AIATestBenchOM(parameter: AIATestBenchParameter) extends Class {
  val aia = IO(Output(Property[AnyClassType]()))
  @public
  val aiaIn = IO(Input(Property[AnyClassType]()))
  aia := aiaIn
}

class AIATestBenchInterface(parameter: AIATestBenchParameter) extends Bundle {
  val om = Output(Property[AnyClassType]())
}

@instantiable
class AIATestBench(val parameter: AIATestBenchParameter)
    extends FixedIORawModule(new AIATestBenchInterface(parameter))
    with SerializableModule[AIATestBenchParameter]
    with ImplicitClock
    with ImplicitReset {
  ...
  // Instantiate DUT.
  val dut: Instance[AIA] = Instantiate(new AIA(parameter.aiaParameter))
  // Instantiate OM
  val omInstance = Instantiate(new AIATestBenchOM(parameter))
  io.om := omInstance.getPropertyReference.asAnyClassType
  omInstance.aiaIn := dut.io.om
  ...
  val watchdogCode = RawUnclockedNonVoidFunctionCall("aia_watchdog", UInt(8.W))(callWatchdog)
